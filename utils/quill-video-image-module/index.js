...
export class ImageExtend {
  ...
  constructor(quill, config = {}) {
    QuillID = Math.ceil(Math.random() * 1000000000000)
    this.id = QuillID
    this.quill = quill
...
export const QuillVideoWatch = {
  watcher: {}, // 登记编辑器信息
  active: null, // 当前触发的编辑器
  on: function (videoExtendId, VideoExtend) {
    // 登记注册使用了videoEXtend的编辑器
...
export class VideoExtend {
  ...
  insertVideo() {
    ...
    self.quill.insertEmbed(
      QuillVideoWatch.active.cursorIndex,
      'video',
      self.videoURL
    )
    self.quill.update()
  ...
  progress(pro) {
    pro = '[' + 'uploading' + pro + ']'
    QuillVideoWatch.active.quill.root.innerHTML = QuillVideoWatch.active.quill.root.innerHTML.replace(
      /\[uploading.*?\]/,
      pro
    )
  }
  ...
  uploading() {
    let length =
      (QuillVideoWatch.active.quill.getSelection() || {}).index ||
      QuillVideoWatch.active.quill.getLength()
    QuillVideoWatch.active.cursorIndex = length
    QuillVideoWatch.active.quill.insertText(
      QuillVideoWatch.active.cursorIndex,
      '[uploading...]',
      {
        color: 'red',
